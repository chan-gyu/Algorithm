...
public class Solution45 {
	public String solution(String new_id) {
        ...
        //6. 길이가 16자 이상이면 첫 15개의 문자를 제외한 나머지 문자제거 ->
        //	 제거 후 마지막문자가 마침표(.) 면 마침표까지 제거
        if(new_id.length()>=16) {
        ...
        //7. 길이가 2자 이하라면 new_id의 마지막 문자를 new_id 길이가 3이 될 때까지 반복
        if(new_id.length()<=2) {
        	while(new_id.length()<3) {
        		System.out.println(new_id.length());
        		String  str = "";
        		if(new_id.length()==1) {
        			str=new_id.substring(0,1);
        		}else {
        			str = new_id.substring(new_id.length()-1, new_id.length());
        		}
        		new_id = new_id+str;
        	}
        }
	...
	"네오"에게 주어진 첫 업무는 새로 가입하는 유저들이 카카오 아이디 규칙에 맞지 않는 아이디를
	입력했을 때, 입력된 아이디와 유사하면서 규칙에 맞는 아이디를 추천해주는 프로그램을 개발하는
	것입니다. 다음은 카카오 아이디의 규칙입니다.
	...
	추천해 주려고 합니다. 신규 유저가 입력한 아이디가 new_id 라고 한다면,
	...
	6단계 new_id의 길이가 16자 이상이면, new_id의 첫 15개의 문자를 제외한 나머지 문자들을 모두
	      제거합니다. 만약 제거 후 마침표(.)가 new_id의 끝에 위치한다면 끝에 위치한
	      마침표(.) 문자를 제거합니다.
	7단계 new_id의 길이가 2자 이하라면, new_id의 마지막 문자를 new_id의 길이가 3이 될 때까지
		  반복해서 끝에 붙입니다.예를 들어, new_id 값이 "...!@BaT#*..y.abcdefghijklm" 라면,
